...
module.exports.superUser = async function (req, res) {
  ...
  try {
    ...
    await User.create({
      userType: "admin",
      name: "abcd",
      email: "yaduvanshiabhi083@gmail.com",
      password: "user123",
      confirmPassword: "user123",
      department: "HR",
    });
    return res.end(
      `<h3>user credentials</h3> <p>email: yaduvanshiabhi083@gmail.com  <br> paasword :  user123</p> <h2>please delete user123 as soon as you create new user</h2> </h2> `
    );
  } catch (error) {
    console.log(error);
...
module.exports.update = async function (req, res) {
  try {
    ...
    if (
      req.body["userType"] &&
      (req.body["userType"] === "employee" || req.body["userType"] === "admin")
    ) {
  ...
  try {
    ...
    if (reviewedUser && req.body.asign) {
      if (typeof req.body.asign === "string") {
        await User.findByIdAndUpdate(req.body.asign, {
          $push: { listToReview: reviewedUser.id },
        });
        return res.redirect("back");
      }
